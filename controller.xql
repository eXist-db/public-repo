...
declare variable $isGet := request:get-method() eq "GET";
declare variable $isPost := request:get-method() eq "POST";
...
) else if ($isGet and $exist:path eq "/search") then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
) else if ($isGet and $exist:path eq "/list") then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
) else if ($isGet and $exist:path eq "/public/apps.xml") then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
) else if ($isGet and $exist:path eq "/feed.xml") then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{$exist:controller}/modules/feed.xq"/>
    </dispatch>

) else if ($isGet and contains($exist:path, "/resources/")) then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <set-header name="Cache-Control" value="max-age=3600, must-revalidate"/>
    </dispatch>
...
(: User is required to be logged in and member of a specific group. Redirect unauthorized requests to the login page. :)
        ...
        $exist:path = ("/admin", "/stats", "/publish")
) then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
) else if (($isGet or $isPost) and $exist:path eq "/admin") then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
) else if (($isGet or $isPost) and $exist:path eq "/stats") then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
...
) else if ($isPost and $exist:path eq "/publish") then (
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
